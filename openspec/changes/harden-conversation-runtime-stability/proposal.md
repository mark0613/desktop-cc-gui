...
- 在 `turn/started` 之后新增一层前端 no-activity watchdog：若 20 秒内未收到 `delta`、`processing/heartbeat`、item lifecycle 更新、`turn/error` 或 `turn/completed`，前端主动结束 processing、清空 active turn，并向用户展示 recoverable timeout message，防止旧会话或僵死 runtime 把 UI 永久卡在 loading。
